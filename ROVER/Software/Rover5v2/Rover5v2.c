...
**********************************************************************************************
**																							**
**  Autor		:	Andr�s David Su�rez G�mez												**
**  Tutor		:	Wilson Javier P�rez Holgu�n												**
**  Instituci�n	: 	Universidad Pedag�gica y Tecnol�gica de Colombia (UPTC)					**
**  Entidad		:	Grupo de Investigacion en Rob�tica y Automatizaci�n Industrial (GIRA)	**
**  Descripci�n :	C�digo para el procesador NIOS II que comanda a un robot desarrollado	**
**  				por el grupo de investigaci�n GIRA con aplicaci�n a rob�tica educativa	**
...
** Function:    main()													**
**																		**
** Description: Funci�n principal del robot m�vil						**
** 																		**
** Notes:       En esta funci�n se hacen los llamados a las demas 		**
...
int main(){
	...
	while(1){
		...
		** direcci�n espec�fica indefinidamente					**
		*********************************************************/
		/*
		if (entradaxbee == 'w'){
			IOWR(MOTORES_BASE,0,ADELANTE);
...
**																		**
** Description: Funci�n para mover el robot en l�nea recta hacia	 	**
** 				adelante a una velocidad constante						**
** 																		**
** Notes:       En esta funci�n se se define un char *mensaje, en donde	**
** 				mensaje es un apuntador que recorre los caracteres del 	**
** 				mensaje que se quiere enviar.							**
**																		**
** Returns:     N/A														**
...
void moveForward(float distance, float velocity){
	float initialDistance = x;
	while (initialDistance - x < distance){
		IOWR(MOTORES_BASE,0,ADELANTE);
		IOWR(PWM2_BASE,0,velocity);
		IOWR(PWM1_BASE,0,velocity);
	}
}
